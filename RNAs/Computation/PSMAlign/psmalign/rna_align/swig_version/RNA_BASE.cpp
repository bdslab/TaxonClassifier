...
int RNA_SEQ::create(string stem_base_seq_str, vector < vector < int > > stem_base_pairs) {
  RNA_BASE base;

  for (int i = 0; i < stem_base_seq_str.size(); i++) {
    base.setbase(stem_base_seq_str[i]);
    //if(base.getbase()>=4)
    //  continue; //not a proper character.
    push_back(base);
  }
  ...
  for (int i = 0; i < stem_base_pairs.size(); i++) {
    int pair_upstream_pos = stem_base_pairs[i][0];
    int pair_downstream_pos = stem_base_pairs[i][1];
    (*this)[pair_upstream_pos].setpairoff(pair_downstream_pos - pair_upstream_pos);
    (*this)[pair_downstream_pos].setpairoff(pair_upstream_pos - pair_downstream_pos);
  }
